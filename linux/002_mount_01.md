...
###挂载虚拟硬盘
loop device另一种常用的用法是虚拟一个硬盘，比如我想玩玩btrfs这个文件系统，但我系统中目前的所有分区都已经用了，里面都是有用的数据，不想格式化他们，这时虚拟硬盘就有用武之地了，示例如下
```bash
...
dev@ubuntu:~$ ls ./bind/bind1/sub1/
aaa

#我们可以在bind2目录下看到刚创建的文件
dev@ubuntu:~$ ls ./bind/bind2/sub1/
aaa
