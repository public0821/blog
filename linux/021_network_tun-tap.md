...
在现在的云时代，虚拟机和容器大规模部署，它们背后的网络管理都离不开虚拟网络设备，所以了解虚拟网络设备有利于我们更好的理解云时代的网络结构。从本篇开始，将介绍Linux下的虚拟网络设备。

## 虚拟设备和物理设备的区别

前面的文章介绍了[Linux网络数据包的接收过程](https://segmentfault.com/a/1190000008836467)和[数据包的发送过程](https://segmentfault.com/a/1190000008926093)，从介绍中可知，Linux内核中有一个网络设备管理层，处于网络设备驱动和协议栈之间，负责衔接它们之间的数据交互。驱动不需要了解协议栈的细节，协议栈也不需要了解设备驱动的细节。

对于一个网络设备来说，就像一个管道（pipe）一样，有两端，从其中任意一端收到的数据将从另一端发送出去。

比如一个物理网卡eth0，它的两端就是内核协议栈（通过内核网络设备管理模块间接的通信）和外面的物理网络，从物理网络收到的数据，会转发给内核协议栈，而应用程序从协议栈过来的数据将会通过物理网络发送出去。
...
tun0是一个Tun/Tap虚拟设备，从上图中可以看出它和物理设备eth0的差别，它们的一端虽然都连着协议栈，但另一端不一样，eth0的另一端是物理网络，这个物理网络可能就是一个交换机，而Tun/Tap设备的另一端是一个用户层的程序，协议栈发给Tun/Tap设备的数据包能被这个应用程序读取到，并且应用程序能直接向Tun/Tap设备写数据。

这里假设eth0配置的IP是192.168.1.11，而tun0配置的IP是192.168.3.11.

> 这里列举的是一个典型的tun/tap设备的应用场景，当然应用程序B收到数据包后可以干任何事情，可以不走后面的5~8步，后面我们会给一个啥事不干的应用程序B的例子
...
5. 进程B收到数据包之后，做一些跟业务相关的处理，然后构造一个新的数据包，将原来的数据包嵌入在新的数据包中，最后通过socket B将数据包转发出去，新数据包的目的IP地址已经变成了一个其它的地址，比如是192.168.10.1.
6. socket B将数据包丢给协议栈
7. 协议栈根据本地路由，发现这个数据包应该要通过eth0发送出去，于是将数据包交给eth0
8. eth0通过物理网络将数据包发送出去

192.168.10.1发送回来的数据包将会由原路径经过应用程序B，并最后返回给应用程序A，

> 这里不讨论Tun/Tap设备tun0是怎么和用户层的进程B进行通信的，对于Linux内核来说，有很多种办法来让内核空间和用户空间的进程进行数据交换。
...
## tun和tap的区别
用户层程序通过tun设备只能读写IP数据包，而通过tap设备能读写链路层数据包，类似于普通socket和raw socket的差别一样，能处理数据包的位置不一样。
...
#--------------------------第三个shell窗口----------------------
#启动之后，通过ip link命令就会发现系统多了一个tun设备，
